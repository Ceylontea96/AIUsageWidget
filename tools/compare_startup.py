"""Compare working-tree usage_widget.py with a Git version, alternating runs.

Only usage_widget.py differs between packages; all other files and probes match.
Run with the same Python that setup_and_run.ps1 selects, as the desktop user.
"""
import argparse
...
def main():
    ...
    baseline = subprocess.check_output(['git', '-c', f'safe.directory={PROJECT.as_posix()}',
        'show', f'{args.baseline_ref}:usage_widget.py'], cwd=PROJECT)
    ...
    (before / 'widget_app.py').write_bytes(baseline)
    ...
    payload = {'baseline_ref': args.baseline_ref,
        'widget_sha256': {key: hashlib.sha256((path / 'widget_app.py').read_bytes()).hexdigest()
